		...
		<div id="total-events-wrap">
			<?php
				...
				if(isset($_POST['location'])){
					...
					$addressDetails = preg_split ("/\, /", $address); 
					$street = "'" .$addressDetails[0]. "'";
					$city = $addressDetails[1];
					$state = $addressDetails[2];


					$stmt = $conn->prepare("SELECT * FROM Challenges WHERE state = ? AND city = ?");
				...
				function createHTMLObj($place){
					echo "<div class='event-wrap' id='". $place['ID'] ."'>" .
						 	"<div id='challenge-info'>" .
						 		"<h2>" . $place['challenge_name'] . "</h2>" .
						 		"<p>" . $place['challenge_desc'] . "</p>" .
						 	"</div>" .
						 	"<div id='loc-details'>" .
						 		...
						 		"<div class='save-wrap'>
						 			<div class='save-button'> 
						 				<form id='save-challenge-form' class='". $place['ID'] ."'>
						 					<input type='hidden' name='rest_name' value='" .$place['restaurant_name'] . "'>
											...
											<input type='hidden' name='challenge_name' value='" .str_replace("'", "",$place['challenge_name']) . "'>
											<input type='hidden' name='challenge_desc' value='" . str_replace("'", "",$place['challenge_desc']) . "'>
											<input type='hidden' name='ID' value='" . $place['ID']. "'>
						 					<input type='submit' id='true-button' value='Save for later?' required/>".
...
<script type="text/javascript">
	...
	$(window).on('load', function(){
		...
		$('body').on('submit', '#save-challenge-form', function (e) {
			...
			$(dest).empty();
			 ...
			 $.ajax({
			    url: "./saveChallenge.php",
			    type: "POST",
			    data: $(this).serialize(),
			    success: function(data){
			                  if(data === "true"){
			                    $message = $([
			                                  "<div class='success'>Stop was saved successfully!</div>"
			                                 ].join("\n"));
			                  }else{
			                    $message = $([
			                                 "<div class='error'>You already saved this stop.</div>"
			                                 ].join("\n"));
			                  }
			      ...
			      error:function(error){
			        ...
			        $errorMessage = $([
			                  "<div class='error'>There was a problem saving the challenge.</div>"
			                ].join("\n"));
